...
Scan<8>::Scan(const OpKernelInfo& info) : OpKernel(info) {
  ...
  device_helpers_.set_data_to_zero_func = [](void*, size_t) -> Status {
    ORT_NOT_IMPLEMENTED("ExecutionProvider running Scan on non-CPU device must provide this implementation.");
  };
}
